...
windows: 1-2, 2-2.5, 2.5-4, 4-9 GeV) and the named variable:
    compass16.sivers.{h+,h-}.{1<z<2,1<z,2<z}.{dpt,dx,dz}
"""
...
#%% -- h-, 1<z<2 (0.1<z<0.2)
ds = DataSet.empty("SIDIS", name="compass16.sivers.h-.1<z<2.dpt", comment="COMPASS16 SSA-Sivers h-, 0.1<z<0.2 (differential in pt, joined across 4 Q-windows)",
...
ds = DataSet.empty("SIDIS", name="compass16.sivers.h-.1<z<2.dx", comment="COMPASS16 SSA-Sivers h-, 0.1<z<0.2 (differential in x, joined across 4 Q-windows)",
                    reference="1609.07374", normErr=[], isNormalized=False)
_add_compass16_points(ds, path_to_data + "Zgt01ls02_n_x_Siv.dat", "x", (0.1, 0.2), -12)
ds.save_csv(path_to_save + ds.name + ".csv")

ds = DataSet.empty("SIDIS", name="compass16.sivers.h-.1<z<2.dz", comment="COMPASS16 SSA-Sivers h-, 0.1<z<0.2 (differential in z, joined across 4 Q-windows)",
                    reference="1609.07374", normErr=[], isNormalized=False)
_add_compass16_points(ds, path_to_data + "Zgt01ls02_n_z_Siv.dat", "z", (0.1, 0.2), -12)
ds.save_csv(path_to_save + ds.name + ".csv")

#%% -- h-, 1<z (z>0.1)
ds = DataSet.empty("SIDIS", name="compass16.sivers.h-.1<z.dpt", comment="COMPASS16 SSA-Sivers h-, z>0.1 (differential in pt, joined across 4 Q-windows)",
                    reference="1609.07374", normErr=[], isNormalized=False)
_add_compass16_points(ds, path_to_data + "Zgt01_n_pt_Siv.dat", "pt", (0.1, 1.), -12)
ds.save_csv(path_to_save + ds.name + ".csv")

ds = DataSet.empty("SIDIS", name="compass16.sivers.h-.1<z.dx", comment="COMPASS16 SSA-Sivers h-, z>0.1 (differential in x, joined across 4 Q-windows)",
                    reference="1609.07374", normErr=[], isNormalized=False)
_add_compass16_points(ds, path_to_data + "Zgt01_n_x_Siv.dat", "x", (0.1, 1.), -12)
ds.save_csv(path_to_save + ds.name + ".csv")

ds = DataSet.empty("SIDIS", name="compass16.sivers.h-.1<z.dz", comment="COMPASS16 SSA-Sivers h-, z>0.1 (differential in z, joined across 4 Q-windows)",
                    reference="1609.07374", normErr=[], isNormalized=False)
_add_compass16_points(ds, path_to_data + "Zgt01_n_z_Siv.dat", "z", (0.1, 1.), -12)
ds.save_csv(path_to_save + ds.name + ".csv")

#%% -- h-, 2<z (z>0.2)
ds = DataSet.empty("SIDIS", name="compass16.sivers.h-.2<z.dpt", comment="COMPASS16 SSA-Sivers h-, z>0.2 (differential in pt, joined across 4 Q-windows)",
                    reference="1609.07374", normErr=[], isNormalized=False)
_add_compass16_points(ds, path_to_data + "Zgt02_n_pt_Siv.dat", "pt", (0.2, 1.), -12)
ds.save_csv(path_to_save + ds.name + ".csv")

ds = DataSet.empty("SIDIS", name="compass16.sivers.h-.2<z.dx", comment="COMPASS16 SSA-Sivers h-, z>0.2 (differential in x, joined across 4 Q-windows)",
                    reference="1609.07374", normErr=[], isNormalized=False)
_add_compass16_points(ds, path_to_data + "Zgt02_n_x_Siv.dat", "x", (0.2, 1.), -12)
ds.save_csv(path_to_save + ds.name + ".csv")

ds = DataSet.empty("SIDIS", name="compass16.sivers.h-.2<z.dz", comment="COMPASS16 SSA-Sivers h-, z>0.2 (differential in z, joined across 4 Q-windows)",
                    reference="1609.07374", normErr=[], isNormalized=False)
